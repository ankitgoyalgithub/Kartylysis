...
def exportMessageToCSVWithFilter(request):
    start_time = long(request.GET['start_time'])/1000
    end_time = long(request.GET['end_time'])/1000
    ...
    for message in Messages.objects.raw(
            "SELECT * FROM SMS_messages where UNIX_TIMESTAMP(STR_TO_DATE(message_time,'%%d/%%m/%%Y  %%H:%%i')) > "+start_time +" AND UNIX_TIMESTAMP(STR_TO_DATE(message_time,'%%d/%%m/%%Y  %%H:%%i')) < "+end_time +" Order by id desc"):
        writer.writerow(
